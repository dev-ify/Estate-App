...
examples().then((user)=>{
   if(user == undefined){
        req.flash("errors","Username Doesnt Exist")
   }
 else{
   bcrypt.compare(password, user[0].password, (err, results) => {
        ...
        if (results === false) {
        req.flash("errors","Password Not Correct ")
...
if(user[0].is_CornerShopManager || user[0].is_ResidentManager || user[0].is_RevenueManager ||
...
is_SuperAdmin=1
       ...
       const sql = `INSERT INTO user(fullname,username,email,telephone,password,is_SuperAdmin,is_ResidentManager,is_RevenueManager,is_ShuttleManager,is_CornerShopManager,status,role) VALUES ( ${fullname},${username},${email},${telephone},${password},${is_SuperAdmin},${is_ResidentManger},${is_RevenueManger},${is_ShuttleManager},${is_CornerShopManger},"Active",${body_roles})`;
      connection.query(sql, (err, result)=> {
      console.log(err)
